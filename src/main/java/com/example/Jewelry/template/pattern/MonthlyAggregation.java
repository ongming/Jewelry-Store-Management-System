...
public class MonthlyAggregation implements AggregationStrategy {

    @Override
    public ReportResult aggregate(List<ReportRow> data, String title, LocalDate start, LocalDate end) {
        Map<YearMonth, List<ReportRow>> grouped = new LinkedHashMap<>();
        if (data != null) {
            for (ReportRow row : data) {
                if (row == null || row.getOccurredAt() == null) {
                    continue;
                }
                YearMonth key = YearMonth.from(row.getOccurredAt());
                grouped.computeIfAbsent(key, ignored -> new ArrayList<>()).add(row);
            }
        }

        List<ReportRow> rows = new ArrayList<>();
        for (Map.Entry<YearMonth, List<ReportRow>> entry : grouped.entrySet()) {
            ...
            LocalDate monthStart = entry.getKey().atDay(1);
            rows.add(new ReportRow(entry.getKey().toString(), total, count, monthStart.atStartOfDay()));
        }
